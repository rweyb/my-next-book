...
const MyBooksButton = ({ bookId, bookObj = {} }) => {
  ...
  // `ownedBooks` 内の `undefined` 要素をフィルタリング
  const validOwnedBooks = ownedBooks.filter(book => book && book.id);

  // 現在の本が本棚にあるかどうかを確認
  const isOwned = validOwnedBooks.some(book => book.id === bookId);
  console.log("Checking bookId:", bookId, "in ownedBooks:", validOwnedBooks);
  ...
  const handleClick = async (event) => {
    ...
    setOwnedBooks((prevOwnedBooks) => {
      // undefined の要素をフィルタリング
      const filteredBooks = prevOwnedBooks.filter(book => book !== undefined);
      const updatedBooks = isOwned
        ? filteredBooks.filter((book) => book.id !== bookId)
        : [...filteredBooks, { id: bookId, ...requestBody }];
